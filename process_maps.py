...
SYSTEM_PROMPT = (
    ...
    "navigation graphs. Visually inspect the floor plan image and:\n"
    "1. Identify every room, point of interest, and hallway junction.\n"
    "2. For each, emit one node placed at the VISUAL CENTER of that room/junction, "
    "with a concise human-readable name (e.g. 'Room 101', 'Lobby', 'North Junction').\n"
    "3. Map the walkable connectivity: emit an edge for every pair of nodes a person "
    "can walk directly between (through a door or along a hallway segment). Do NOT "
    "connect rooms that are separated by a wall with no doorway.\n"
    "Coordinates are in PIXELS of the provided image, origin (0,0) at the TOP-LEFT, "
    "x increasing right, y increasing down. Every x must be within [0, width) and "
    "every y within [0, height). Node ids must be unique strings; every edge source "
    "and target must reference an existing node id."
)
